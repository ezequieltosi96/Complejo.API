...
{
    public class GetAllUserByFilterHandler : IRequestHandler<GetAllUserByFilterQuery, PagedListResponse<IList<UserByFilterDto>>>
    {
        private readonly IUserService userService;
        private readonly IMapping mapping;

        public GetAllUserByFilterHandler(IUserService userService, IMapping mapping)
        {
            this.userService = userService;
            this.mapping = mapping;
        }

        public async Task<PagedListResponse<IList<UserByFilterDto>>> Handle(GetAllUserByFilterQuery request, CancellationToken cancellationToken)
        {
            PagedList<Models.Identity.User.User> users = await userService.GetAllUserByFilter(request.SearchCriteria, request.Page, request.Size);
